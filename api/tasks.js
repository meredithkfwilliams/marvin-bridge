...
async function collectChildren(parentId, categoryPath) {
    let children;
    try {
        children = await fetchMarvin(`/children?parentId=${parentId}`, apiToken);
        await sleep(200);
    } catch (err) {
...
export default async function handler(req, res) {
  ...
  const missingStars = todayTasks.filter((t) => !isRedOrOrange(t));
  content += `# Today - ${today}\n\n`;
  if (actionable.length === 0) content += "_No actionable items for today._\n";
  else for (const t of actionable) content += taskToMarkdown(t, labelMap) + "\n";
  if (missingStars.length > 0) {
    content += `\n## ⚠️ Missing Star\n`;
  ...
  content += `\n# On Deck\n\n`;
  if (ondeck.length === 0) content += "_Nothing on deck._\n";
  else for (const t of ondeck) content += taskToMarkdown(t, labelMap) + "\n";
  ...
  const upcoming = allTasks
    ...
    .sort((a, b) => a.day.localeCompare(b.day));
  content += `\n# Upcoming\n\n`;
  if (upcoming.length === 0) content += "_Nothing scheduled ahead._\n";
  else for (const t of upcoming) content += taskToMarkdown(t, labelMap) + "\n";
  ...
  content += `\n# Just For Me\n\n`;
  if (!selfId) content += `_⚠️ Could not find "self" label._\n`;
  else if (justforme.length === 0) content += "_Nothing here right now._\n";
  else for (const t of justforme) content += taskToMarkdown(t, labelMap) + "\n";
  ...
  content += `\n# Everything Else\n\n`;
  if (everything.length === 0) content += "_Nothing here._\n";
  else for (const t of everything) content += taskToMarkdown(t, labelMap) + "\n";
  ...
  return res.status(200).send(`✅ Refreshed at ${new Date().toISOString()}`);
      ...
      return res.status(400).json({
        error: `Unknown view: ${view}. Use: today, ondeck, upcoming, justforme, everything, all, categories, category, labels, debug`,
      });
    }
